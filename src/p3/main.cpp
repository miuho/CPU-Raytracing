...
static void print_usage( const char* progname )
{
	...
	" height] [-o output_file]\n"
        ...
        "\t-b:\n" \
        "\t\tAllow lights attenuate in dielectrics according to Beer's Law.\n" \
        "\t-d width height\n" \
        ...
        "\t-c camera_len_length:\n" \
        "\t\tThe side length of camera len to create depth of field.\n" \
        "\toutput_file:\n" \
...
static bool parse_args( Options* opt, int argc, char* argv[] )
{
        ...
        for (int i = 2; i < argc; i++)
	{
		...
		case 'c':
			if (i < argc - 1)
				opt->camera_len_length = atoi(argv[++i]);
			break;
		case 'n':
			if (i < argc - 1)
    ...
    // in order to create depth of field effect, the number of samples per 
    // pixel cannot be just 1.
    if (opt->camera_len_length != 0 && opt->num_samples == 1) {
        opt->num_samples = 3;
    }
